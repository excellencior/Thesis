...
def get_args_parser():
    ...
    parser.add_argument('--num_frames', type=int, help='number of prompt frames in video',
                        default=0)
    parser.add_argument('--prompt_image', type=str, nargs='+', help='paths to prompt images',
                        default=None)
    parser.add_argument('--prompt_target', type=str, nargs='+', help='paths to prompt targets',
                        default=None)
...
if __name__ == '__main__':
    ...
    assert args.input_image or args.input_video, "Provide either an input image or an input video"
    assert not (args.input_image and args.input_video), "Provide only one input type: image or video"

    if args.input_image is not None:
        assert args.prompt_image is not None and args.prompt_target is not None, "Provide prompt images and targets"
        
        img_name = os.path.basename(args.input_image)
        out_path = os.path.join(args.output_dir, "output_" + '.'.join(img_name.split('.')[:-1]) + '.png')

        # Call inference_image with the list of prompt images and targets
        inference_image(model, device, args.input_image, args.prompt_image, args.prompt_target, out_path)
    
    if args.input_video is not None:
        assert args.prompt_target is not None and len(args.prompt_target) == 1, "Provide a single prompt target for video"
        
        vid_name = os.path.basename(args.input_video)
        out_path = os.path.join(args.output_dir, "output_" + '.'.join(vid_name.split('.')[:-1]) + '.mp4')

        # Call inference_video with the list of prompt images
