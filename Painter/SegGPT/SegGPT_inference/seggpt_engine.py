...
@torch.no_grad()
def run_one_image(imgs, tgts, model, device):
    x = torch.tensor(imgs)
    ...
    bool_masked_pos = bool_masked_pos.unsqueeze(dim=0).repeat(x.size(0), 1)
    ...
    output = y[:, y.shape[1]//2:, :, :].mean(dim=0)
...
def inference_image(model, device, img_path, img2_paths, tgt2_paths, out_path):
    ...
    for img2_path, tgt2_path in zip(img2_paths, tgt2_paths):
        ...
        # Prepare input and target by concatenating
